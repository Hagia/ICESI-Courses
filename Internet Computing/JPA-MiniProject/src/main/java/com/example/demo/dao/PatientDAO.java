...
public class PatientDAO implements IPatientDAO {
	...
	public List<Patient> findAllByName(String name, String sortingCriteria) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT a ")
			.append("FROM Patient a, Urgency b ")
			.append("WHERE b.patient=a.id ")
			.append("ORDER BY a." + sortingCriteria + " ");
		return manager.createQuery(sb.toString()).getResultList();
	...
	public List<Patient> findAllTwoUrgencies() {
		...
		sb.append("SELECT p ")
				...
				.append("( SELECT pa.id ")
				.append("FROM Urgency u, Patient pa ")
				.append("WHERE pa.id=u.patient ")
				.append("AND u.date >= :LookBack ")
				.append(") AND COUNT(p.id)>2 ");
